 ...
 */
class GeocodeParser internal constructor() {
    ...
    fun parseGeocodeResponse(string: String): List<Address> {
        ...
        for (i in 0 until jArray.length()) {
            try {
                ...
                val locality = addressJson.getString("locality")
                val postalCode = addressJson.getString("postal_code")
                val adminArea = addressJson.getString("admin_1")
                val neighborhood = addressJson.getString("neighborhood")
                val streetAddress = addressJson.getString("street_address")
                val statusCode: LocationStatus? =
                        if (addressJson.has("status-code")) {
                ...
                val entrances =
                        if (addressJson.has("entrances")) {
                            parseEntrances(addressJson.getJSONArray("entrances"))
                        } else {
                            null
                        }

                val (lat, lon) = parseLocation(addressJson.getJSONObject("location"))
