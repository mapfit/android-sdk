...
// JNI Env bound on androids render thread (our native main thread)
    ...
    void AndroidPlatform::setupJniEnv(JNIEnv *jniEnv) {
        ...
        getFontFilePath = jniEnv->GetMethodID(tangramClass, "getFontFilePath",
                                              "(Ljava/lang/String;)Ljava/lang/String;");
        getFontFallbackFilePath = jniEnv->GetMethodID(tangramClass, "getFontFallbackFilePath",
        ...
        hashmapPutMID = jniEnv->GetMethodID(hashmapClass, "put",
    ...
    jstring jstringFromString(JNIEnv *jniEnv, const std::string &string) {
        ...
        auto chars = std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>().from_bytes(
                string);
    ...
    std::string AndroidPlatform::fontPath(const std::string &_family, const std::string &_weight,
                                          const std::string &_style) const {
        ...
        jstring returnStr = (jstring) jniEnv->CallObjectMethod(m_tangramInstance, getFontFilePath,
                                                               jkey);
    ...
    FontSourceHandle
    AndroidPlatform::systemFont(const std::string &_name, const std::string &_weight,
    ...
    bool AndroidPlatform::bytesFromAssetManager(const char *_path,
                                                std::function<char *(size_t)> _allocator) const {
    ...
    void AndroidPlatform::onUrlComplete(JNIEnv *_jniEnv, jlong _jRequestHandle, jbyteArray _jBytes,
                                        jstring _jError) {
        ...
        if (_jBytes != nullptr) {
            ...
            _jniEnv->GetByteArrayRegion(_jBytes, 0, length,
                                        reinterpret_cast<jbyte *>(response.content.data()));
    ...
    void labelPickCallback(jobject listener, const Tangram::LabelPickResult *labelPickResult) {
        ...
        if (labelPickResult) {
            ...
            labelPickResultObject = jniEnv->NewObject(labelPickResultClass, labelPickResultInitMID,
                                                      labelPickResult->coordinates.longitude,
                                                      labelPickResult->coordinates.latitude,
                                                      labelPickResult->type, hashmap);
        }

        jniEnv->CallVoidMethod(listener, onLabelPickMID, labelPickResultObject, position[0],
                               position[1]);
        jniEnv->DeleteGlobalRef(listener);
    }

    void markerPickCallback(jobject listener, jobject tangramInstance,
                            const Tangram::MarkerPickResult *markerPickResult) {
        ...
        if (markerPickResult) {
            ...
            marker = jniEnv->CallObjectMethod(tangramInstance, markerByIDMID,
                                              static_cast<jlong>(markerPickResult->id));
        ...
        jniEnv->CallVoidMethod(listener, onMarkerPickMID, markerPickResultObject, position[0],
                               position[1]);
        jniEnv->DeleteGlobalRef(listener);
    ...
    void
    featurePickCallback(jobject listener, const Tangram::FeaturePickResult *featurePickResult) {
